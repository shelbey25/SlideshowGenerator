...
class PDFGenerator {
    ...
    /**
     * Generate a PDF slideshow with face-aware cropping, borders, and crop overrides.
     * options may include: bgColor, quality, slideSize, borderWidth, borderColor,
     *                      cropOverrides (Map), previewSlideData (Array)
     */
    static async generate(photos, layoutSequence, options, onProgress) {
        ...
        const borderRGB = borderWidth > 0 ? PDFGenerator._hexToRgb(options.borderColor || '#ffffff') : null;

        let slideDataList;

        if (options.previewSlideData && options.previewSlideData.length > 0) {
            // Reuse precomputed slide data from preview
            slideDataList = options.previewSlideData;
            onProgress(0.10, 'Using preview layout...');
        } else {
            // Build fresh slide data
        ...
        for (let si = 0; si < slideDataList.length; si++) {
            ...
            for (let fi = 0; fi < sd.layout.frames.length && fi < sd.matchedPhotos.length; fi++) {
                ...
                const pct = 0.11 + (photosDone / totalPhotos) * 0.87;
...
class PreviewRenderer {
    /**
     * Render a slide preview with face-aware cropping, borders, and crop overrides.
     * opts = { cropOverrides, borderWidth, borderColor, skipMatching }
     */
    static async renderSlide(photos, layout, bgColor, canvasWidth, slideRatio, opts) {
        const ratio = slideRatio || (16 / 9);
        const canvasHeight = Math.round(canvasWidth / ratio);
        const cropOverrides = opts && opts.cropOverrides || null;
        const borderWidth = opts && opts.borderWidth || 0;
        const borderColor = opts && opts.borderColor || '#ffffff';
        const skipMatching = opts && opts.skipMatching;
        ...
        if (skipMatching) {
            ...
            matched = imageAnalyzer.matchPhotosToFrames(photos, layout.frames, ratio);
        }
        ...
        for (let i = 0; i < layout.frames.length && i < matched.length; i++) {
            ...
            try {
                ...
                // Get focus point (manual override or face detection)
                let focus;
                if (cropOverrides && cropOverrides.has(photo.file)) {
                    focus = cropOverrides.get(photo.file);
                } else {
                ...
                if (borderWidth > 0) {
                    ...
                    ctx.lineWidth = Math.max(1, borderWidth * (canvasWidth / 960));
                    ctx.strokeRect(
                        fx + ctx.lineWidth / 2,
                        fy + ctx.lineWidth / 2,
                        fw - ctx.lineWidth,
        ...
        for (let i = matched.length; i < layout.frames.length; i++) {
            const frame = layout.frames[i];
...
class App {
    constructor() {
        ...
        this._cropOverrides = new Map();
        this._previewSlideData = null;
        this._recropCleanup = [];
    ...
    _bindGenerate() {
        ...
        // Border controls
        document.getElementById('border-width').addEventListener('change', () => this._updateSummary());
        const borderColorInput = document.getElementById('border-color');
        const borderColorHex = document.getElementById('border-color-hex');
        borderColorInput.addEventListener('input', () => {
            borderColorHex.textContent = borderColorInput.value;
            this._updateSummary();
        });
    ...
    _updateSummary() {
        ...
        summaryEl.innerHTML = `
            ...
            Borders: <strong>${document.getElementById('border-width').value === '0' ? 'None' : document.getElementById('border-width').value + 'pt'}</strong>
            <span style="display:inline-block;width:14px;height:14px;border-radius:3px;vertical-align:middle;border:1px solid #ccc;background:${document.getElementById('border-color').value}"></span> |
    ...
    async _preview() {
        ...
        previewSlides.innerHTML = '<p class="preview-progress" id="preview-progress">Analyzing photos...</p>';

        await new Promise(r => setTimeout(r, 50));

        const photos = this._getOrderedPhotos();
        const bgColor = document.getElementById('bg-color').value;
        const borderWidth = parseFloat(document.getElementById('border-width').value) || 0;
        const borderColor = document.getElementById('border-color').value;
        const slideSizeKey = document.getElementById('slide-size').value;
        const slideSize = SLIDE_SIZES[slideSizeKey] || SLIDE_SIZES['16:9'];
        const slideRatio = slideSize.width / slideSize.height;

        // Phase 1: Analyze all photos
        await imageAnalyzer.analyzeAll(photos, (pct) => {
            const el = document.getElementById('preview-progress');
            if (el) el.textContent = `Analyzing photos... ${Math.round(pct * 100)}%`;
        });

        // Phase 2: Detect faces
        const progressEl = document.getElementById('preview-progress');
        if (progressEl) progressEl.textContent = 'Detecting faces...';
        await faceAwareCropper.analyzeAll(photos, (pct) => {
            const el = document.getElementById('preview-progress');
        ...
        const totalSlides = this._previewSlideData.length;

        for (let i = 0; i < totalSlides; i++) {
            const sd = this._previewSlideData[i];

            try {
                const canvas = await PreviewRenderer.renderSlide(
                    sd.matchedPhotos, sd.layout, bgColor, 280, slideRatio,
                    { cropOverrides: this._cropOverrides, borderWidth, borderColor, skipMatching: true }
                );
                canvas.className = 'preview-slide';

                const wrapper = document.createElement('div');
                wrapper.className = 'preview-slide-wrapper';
                wrapper.dataset.slideIdx = i;
                wrapper.addEventListener('click', () => this._openRecropModal(i));
                ...
                console.warn('Preview error slide', i, err);
            }

            // Yield every 3 slides for UI update
            if (i % 3 === 0) {
                await new Promise(r => setTimeout(r, 0));
            }
        }
    ...
    _bindRecrop() {
        document.getElementById('recrop-close').addEventListener('click', () => this._closeRecropModal());
        document.getElementById('recrop-done-btn').addEventListener('click', () => this._closeRecropModal());
        document.getElementById('recrop-prev-btn').addEventListener('click', () => this._goRecropSlide(-1));
        document.getElementById('recrop-next-btn').addEventListener('click', () => this._goRecropSlide(1));
        document.getElementById('recrop-reset-btn').addEventListener('click', () => this._resetRecropSlide());

        // Close on overlay click
        document.getElementById('recrop-modal').addEventListener('click', (e) => {
            if (e.target.id === 'recrop-modal') this._closeRecropModal();
        });

        // Keyboard navigation
        document.addEventListener('keydown', (e) => {
            if (document.getElementById('recrop-modal').style.display === 'none') return;
            if (e.key === 'Escape') this._closeRecropModal();
            if (e.key === 'ArrowLeft') this._goRecropSlide(-1);
            if (e.key === 'ArrowRight') this._goRecropSlide(1);
        });
    }

    _openRecropModal(slideIdx) {
        if (!this._previewSlideData || slideIdx < 0 || slideIdx >= this._previewSlideData.length) return;

        this._recropSlideIdx = slideIdx;
        const modal = document.getElementById('recrop-modal');
        modal.style.display = '';

        this._updateRecropNav();
        this._renderRecropSlide();
    }

    _closeRecropModal() {
        this._cleanupRecropListeners();
        document.getElementById('recrop-modal').style.display = 'none';

        // Refresh the preview thumbnail for the current slide
        if (this._recropSlideIdx != null) {
            this._refreshPreviewSlide(this._recropSlideIdx);
        }
    }

    _goRecropSlide(delta) {
        const newIdx = this._recropSlideIdx + delta;
        if (newIdx < 0 || newIdx >= this._previewSlideData.length) return;

        // Refresh old slide thumbnail before moving
        this._refreshPreviewSlide(this._recropSlideIdx);

        this._cleanupRecropListeners();
        this._recropSlideIdx = newIdx;
        this._updateRecropNav();
        this._renderRecropSlide();
    }

    _updateRecropNav() {
        const idx = this._recropSlideIdx;
        const total = this._previewSlideData.length;
        document.getElementById('recrop-title').textContent = `Slide ${idx + 1} of ${total}`;
        document.getElementById('recrop-prev-btn').disabled = idx <= 0;
        document.getElementById('recrop-next-btn').disabled = idx >= total - 1;
    }

    _resetRecropSlide() {
        const sd = this._previewSlideData[this._recropSlideIdx];
        for (const photo of sd.matchedPhotos) {
            this._cropOverrides.delete(photo.file);
        }
        this._renderRecropSlide();
    }

    _cleanupRecropListeners() {
        for (const fn of this._recropCleanup) {
            fn();
        }
        this._recropCleanup = [];
    }

    async _renderRecropSlide() {
        this._cleanupRecropListeners();

        const sd = this._previewSlideData[this._recropSlideIdx];
        const container = document.getElementById('recrop-slide');
        container.innerHTML = '';
        ...
        const containerHeight = Math.round(containerWidth / slideRatio);
        container.style.width = containerWidth + 'px';
        container.style.height = containerHeight + 'px';
        container.style.background = bgColor;

        for (let fi = 0; fi < sd.matchedPhotos.length; fi++) {
            const frame = sd.layout.frames[fi];
            const photo = sd.matchedPhotos[fi];

            // Frame pixel dimensions
            const fw = (frame.w / 100) * containerWidth;
            const fh = (frame.h / 100) * containerHeight;

            const frameDiv = document.createElement('div');
            frameDiv.className = 'recrop-frame';
            frameDiv.style.left = (frame.x / 100 * containerWidth) + 'px';
            frameDiv.style.top = (frame.y / 100 * containerHeight) + 'px';
            ...
            let focus;
            if (this._cropOverrides.has(photo.file)) {
                focus = this._cropOverrides.get(photo.file);
            } else {
                focus = await faceAwareCropper.getFocusPoint(photo);
            }

            // Position image within frame
            this._positionRecropImage(img, fw, fh, focus);

            // Make draggable
            this._setupFrameDrag(frameDiv, img, photo, fw, fh);
        }
    }

    _positionRecropImage(img, frameW, frameH, focus) {
        const imgW = img.naturalWidth;
        const imgH = img.naturalHeight;
        if (!imgW || !imgH) return;
        ...
        let imgX = -(focus.fx * renderW) + frameW / 2;
        imgX = Math.min(0, Math.max(frameW - renderW, imgX));

        let imgY = -(focus.fy * renderH) + frameH / 2;
        imgY = Math.min(0, Math.max(frameH - renderH, imgY));

        img.style.left = imgX + 'px';
        img.style.top = imgY + 'px';
    }

    _setupFrameDrag(frameDiv, img, photo, frameW, frameH) {
        let dragging = false;
        ...
        const onDown = (e) => {
            ...
            startX = e.clientX;
            startY = e.clientY;
            startImgX = parseFloat(img.style.left) || 0;
            startImgY = parseFloat(img.style.top) || 0;
            frameDiv.classList.add('dragging');
            frameDiv.setPointerCapture(e.pointerId);
        };
        ...
        const onMove = (e) => {
            ...
            const renderW = parseFloat(img.style.width);
            const renderH = parseFloat(img.style.height);

            let newX = startImgX + dx;
            let newY = startImgY + dy;

            newX = Math.min(0, Math.max(frameW - renderW, newX));
            newY = Math.min(0, Math.max(frameH - renderH, newY));
        ...
        const onUp = (e) => {
            ...
            const fx = Math.max(0, Math.min(1, (-imgX + frameW / 2) / renderW));
            const fy = Math.max(0, Math.min(1, (-imgY + frameH / 2) / renderH));

            this._cropOverrides.set(photo.file, { fx, fy });
        };

        frameDiv.addEventListener('pointerdown', onDown);
        frameDiv.addEventListener('pointermove', onMove);
        frameDiv.addEventListener('pointerup', onUp);

        this._recropCleanup.push(() => {
            frameDiv.removeEventListener('pointerdown', onDown);
            frameDiv.removeEventListener('pointermove', onMove);
            frameDiv.removeEventListener('pointerup', onUp);
        });
    }

    async _refreshPreviewSlide(slideIdx) {
        const sd = this._previewSlideData[slideIdx];
        if (!sd) return;
        ...
        try {
            ...
            const wrapper = document.querySelector(`.preview-slide-wrapper[data-slide-idx="${slideIdx}"]`);
            if (wrapper) {
                const oldCanvas = wrapper.querySelector('.preview-slide');
                if (oldCanvas) wrapper.replaceChild(canvas, oldCanvas);
            }
        } catch (err) {
            console.warn('Refresh preview error:', err);
        }
    }
}
